...
const utils = {
  ...
  validatePhone: (phone: string): string => {
    if (!phone || !NOTIFICATION_CONFIG.sms.phoneRegex.test(phone)) {
      console.warn(
        `[Orchestrator] Invalid phone number: ${phone}, using fallback`
      );
    ...
    for (const channel of params.channels) {
      try {
        ...
        switch (channel.type) {
          ...
          default:
            console.warn(
              `[Orchestrator] Unknown channel type: ${channel.type}`
            );
            continue;
        }
        ...
        console.error(
          `[Orchestrator] Error sending ${channel.type} notification:`,
          error
        );
...
async function logAndSendInAppNotification(notification: any) {
  try {
    ...
    if (!shouldSend) {
      console.log(
        `[IN-APP] In-app notification skipped for user ${notification.user_id} - not allowed`
      );
      return { success: true, skipped: true, reason: "in_app_not_allowed" };
    }

    console.log(
      `[IN-APP] Notification to userId: ${notification.user_id} | message: ${notification.message}`
    );
    return await sendInAppNotification(notification);
  } catch (error) {
    console.error(
      `[IN-APP] Error sending in-app notification to user ${notification.user_id}:`,
      error
    );
...
const eventHandlers: Record<
  NotificationEventType,
  (request: NotificationRequest) => Promise<any>
> = {
  async random_project_recommendation(request: NotificationRequest) {
    ...
    if (error)
      throw new Error(`Failed to fetch random project: ${error.message}`);
    ...
    await logNotification({
      ...
      error: inAppResult.success
        ? undefined
        : JSON.stringify(inAppResult.error),
  ...
  async project_created(request: NotificationRequest) {
    ...
    const dynamicTemplateData = {
      ...projectDetails,
      unsubscribeLink: utils.generateUnsubscribeLink(
        mentorEmail,
        request.userId
      ),
    };
  ...
  async session_reminder(request: NotificationRequest) {
    const {
      message,
      bookingId,
      emailType: reminderEmailType = "transactional",
      channel,
      phoneNumber,
      email,
    } = request.context;
    ...
    const channelHandlers = {
      sms: async () => {
        console.log(
          `[Orchestrator] Sending SMS for session_reminder to ${phoneNumber}`
        );
      ...
      email: async () => {
        console.log(
          `[Orchestrator] Sending Email for session_reminder to ${email}`
        );
      ...
      in_app: async () => {
        console.log(
          `[Orchestrator] Sending In-App for session_reminder to user ${request.userId}`
        );
    ...
    const handler =
      channelHandlers[channel as keyof typeof channelHandlers] ||
      channelHandlers.in_app;
  ...
  async unread_chat_reminder(request: NotificationRequest) {
    ...
    console.log(
      `[Orchestrator] Sending unread chat reminder to user ${userId} with ${unreadCount} unread messages`
    );
  ...
  async chat_message(request: NotificationRequest) {
    ...
    if (recipientResult.error || !recipientResult.data) {
      console.error(
        `[Orchestrator] No recipient profile found for user ${request.userId}. Notification skipped.`
      );
    ...
    if (!existingReminder) {
      ...
      console.log(
        `[Orchestrator] Scheduled unread reminder for user ${request.userId} in channel ${channel_id} at ${sendAt.toISOString()}`
      );
    }
  ...
  async mentor_profile_approved(request: NotificationRequest) {
    console.warn(
      `[Orchestrator] mentor_profile_approved event not implemented yet`
    );
    return { success: false, error: "Event type not implemented" };
  },
};
