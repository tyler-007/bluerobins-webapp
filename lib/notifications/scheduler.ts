...
async function markNotificationAsFailed(
  ...
  console.error(
    `[Scheduler] Failed to send scheduled notification ${notif.id}:`,
    error
  );
}
...
export async function processScheduledNotifications() {
  ...
  if (error) {
    console.error(
      "[Scheduler] Failed to fetch scheduled notifications:",
      error
    );
    return;
  }
  ...
  console.log(
    `[Scheduler] Processing ${notifications.length} scheduled notifications`
  );

  for (const notif of notifications) {
    try {
      const eventType = (notif.context?.eventType ||
        notif.event_type ||
        "session_reminder") as NotificationEventType;
...
export async function cleanupDeletedMessageReminders() {
  ...
  console.log(
    `[Cleanup] Marked ${updatedReminders?.length || 0} reminders as deleted for deleted messages`
  );
}
